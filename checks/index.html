...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="RTW Checks">
    <meta name="theme-color" content="#0d0d0d">
    <title>RTW Checks</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* DASHBOARD */
        .dashboard-header { position: sticky; top: 0; z-index: 100; background: rgba(13, 13, 13, 0.9); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 1px solid var(--border); padding: 20px 24px; padding-top: max(20px, env(safe-area-inset-top)); }
        ...
        /* TILES */
        .tiles-container { padding: 24px; max-width: 600px; margin: 0 auto; }
        ...
        .app-header { position: sticky; top: 0; z-index: 100; background: rgba(13, 13, 13, 0.9); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 1px solid var(--border); padding: 16px 20px; padding-top: max(16px, env(safe-area-inset-top)); }
        ...
        .pdf-section { padding: 20px; max-width: 800px; margin: 0 auto; }
        .pdf-section .btn-primary { max-width: 800px; margin: 0 auto; }

        /* LIGHT MODE */
        :root.light-mode {
            --bg-primary: #f5f5f5; --bg-secondary: #e8e8e8; --bg-card: #ffffff; --bg-input: #f0f0f0;
            --text-primary: #1a1a1a; --text-secondary: #555555; --text-muted: #888888;
            --border: #d0d0d0; --shadow: 0 4px 24px rgba(0,0,0,0.1);
        }
        .theme-toggle { position: fixed; bottom: 20px; right: 20px; width: 44px; height: 44px; border-radius: 50%; background: var(--bg-card); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 9998; box-shadow: var(--shadow); transition: var(--transition); }
        .theme-toggle:hover { transform: scale(1.1); }
        .theme-toggle svg { width: 22px; height: 22px; fill: var(--text-secondary); }

        @media (max-width: 600px) { .tiles-grid { grid-template-columns: 1fr; } .toggle-switch { width: 64px; height: 40px; } .toggle-slider:before { height: 32px; width: 32px; } .toggle-switch input:checked + .toggle-slider:before { transform: translateX(24px); } }
    </style>
...
<body>
        ...
        <header class="dashboard-header">
            <div class="dashboard-header-top">
                <div class="dashboard-logo">
                    <div class="dashboard-logo-icon"><svg viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-1.99.9-1.99 2L3 19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-1 11h-4v4h-4v-4H6v-4h4V6h4v4h4v4z"/></svg></div>
                    <span class="dashboard-logo-text">RTW Checks</span>
                </div>
            </div>
            <div class="dashboard-date" id="dashboardDate"></div>
        </header>
        <div class="tiles-container">
            <div class="tiles-section-title">Checks auswählen</div>
            <div class="tiles-grid">
                <div class="tile blue" onclick="openCheck('mpg')">
                ...
                <div class="tile orange" onclick="openCheck('wochen')">
    ...
    <!-- MPG-CHECK RTW -->
    <div class="screen app-screen" id="mpgScreen">
        <header class="app-header" style="border-left: 4px solid var(--blue);">
        ...
        <div class="pdf-section"><button class="btn btn-primary" onclick="exportPDF('mpg')"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8.5 7.5c0 .83-.67 1.5-1.5 1.5H9v2H7.5V7H10c.83 0 1.5.67 1.5 1.5v1zm5 2c0 .83-.67 1.5-1.5 1.5h-2.5V7H15c.83 0 1.5.67 1.5 1.5v3zm4-3H19v1h1.5V11H19v2h-1.5V7h3v1.5zM9 9.5h1v-1H9v1zM4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm10 5.5h1v-3h-1v3z"/></svg>Als PDF exportieren</button></div>
    </div>

    <!-- WOCHENCHECK RTW -->
    <div class="screen app-screen" id="wochenScreen">
        <header class="app-header" style="border-left: 4px solid var(--orange);">
            <div class="header-top">
                <button class="back-btn" onclick="showScreen('dashboard')"><svg viewBox="0 0 24 24"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg></button>
        ...
        <div class="pdf-section"><button class="btn btn-primary" onclick="exportPDF('wochen')"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8.5 7.5c0 .83-.67 1.5-1.5 1.5H9v2H7.5V7H10c.83 0 1.5.67 1.5 1.5v1zm5 2c0 .83-.67 1.5-1.5 1.5h-2.5V7H15c.83 0 1.5.67 1.5 1.5v3zm4-3H19v1h1.5V11H19v2h-1.5V7h3v1.5zM9 9.5h1v-1H9v1zM4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm10 5.5h1v-3h-1v3z"/></svg>Als PDF exportieren</button></div>
    </div>

    <!-- THEME TOGGLE -->
    <div class="theme-toggle" onclick="toggleTheme()"><svg viewBox="0 0 24 24" id="themeIcon"><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9c0-.46-.04-.92-.1-1.36-.98 1.37-2.58 2.26-4.4 2.26-2.98 0-5.4-2.42-5.4-5.4 0-1.81.89-3.42 2.26-4.4-.44-.06-.9-.1-1.36-.1z"/></svg></div>

    <script>
        // ============ CHECKLIST DATA ============
        var mpgChecklistData = [
        ...
        // ============ STATE ============
        var mpgChecklistState = {}, mpgCommentsState = {}, mpgSignatureData = null;
        var wochenChecklistState = {}, wochenCommentsState = {}, wochenSignatureData = null;
        ...
        function openCheck(type) {
            if (type === 'mpg') { initMpgCheck(); showScreen('mpg'); }
            else if (type === 'wochen') { initWochenCheck(); showScreen('wochen'); }
        }
        ...
        function isLastWeekOfMonth() {
            ...
            return (lastDay.getDate() - now.getDate()) < 7;
        }
        ...
        // ============ GENERIC CHECKLIST ============
        function getChecklistConfig(type) {
            var configs = {
                mpg: {
                    data: mpgChecklistData, stateKey: 'mpgChecklistState', commentsKey: 'mpgCommentsState',
                    containerId: 'mpgChecklistContainer', progressFillId: 'mpgProgressFill',
                    progressCountId: 'mpgProgressCount', totalCountId: 'mpgTotalCount',
                    employeeNameId: 'mpgEmployeeName', commentPrefix: 'mpg-',
                    filterKey: null, filterFn: null, hasDocLinks: false,
                    getSignatureData: function() { return mpgSignatureData; }
                },
                wochen: {
                    data: wochenChecklistData, stateKey: 'wochenChecklistState', commentsKey: 'wochenCommentsState',
                    containerId: 'wochenChecklistContainer', progressFillId: 'wochenProgressFill',
                    progressCountId: 'wochenProgressCount', totalCountId: 'wochenTotalCount',
                    employeeNameId: 'wochenEmployeeName', commentPrefix: 'wochen-',
                    filterKey: 'isLastWeek', filterFn: isLastWeekOfMonth, hasDocLinks: true,
                    getSignatureData: function() { return wochenSignatureData; }
                }
            };
            return configs[type];
        }
        ...
        function getState(key) {
            return { mpgChecklistState: mpgChecklistState, wochenChecklistState: wochenChecklistState }[key];
        }

        function getComments(key) {
            return { mpgCommentsState: mpgCommentsState, wochenCommentsState: wochenCommentsState }[key];
        }

        function renderGenericChecklist(type) {
            var cfg = getChecklistConfig(type);
            var state = getState(cfg.stateKey);
            var c = document.getElementById(cfg.containerId);
            var showFiltered = cfg.filterFn ? cfg.filterFn() : true;
            var h = '';
            cfg.data.forEach(function(s) {
                if (cfg.filterKey && s[cfg.filterKey] && !showFiltered) return;
                h += '<div class="section" data-section="' + s.id + '"><div class="section-header" onclick="toggleSection(\'' + s.id + '\')"><div class="section-icon">' + s.icon + '</div><div class="section-info"><div class="section-title">' + s.title + '</div><div class="section-count" id="count-' + s.id + '">0 / ' + s.items.length + '</div></div><div class="section-toggle"><svg viewBox="0 0 24 24"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg></div></div><div class="section-content">';
                s.items.forEach(function(it, i) {
                    var itemText = typeof it === 'object' ? it.text : it;
                    var docLink = (cfg.hasDocLinks && typeof it === 'object' && it.doc) ? '<a class="doc-link" href="../docs/' + it.doc + '" target="_blank" title="Original-Checkliste \u00f6ffnen" onclick="event.stopPropagation()"><svg viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11z"/></svg></a>' : '';
                    h += '<div class="check-item"><div class="check-item-row"><span class="check-label" id="label-' + s.id + '-' + i + '">' + itemText + '</span>' + docLink + '<button class="comment-toggle" onclick="toggleGenericComment(\'' + type + '\',\'' + s.id + '-' + i + '\')" title="Kommentar hinzuf\u00fcgen"><svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.17L4 17.17V4h16v12zM7 9h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg></button><label class="toggle-switch"><input type="checkbox" data-key="' + s.id + '-' + i + '" onchange="handleGenericCheckChange(\'' + type + '\',this)"><span class="toggle-slider"></span></label></div><div class="comment-field" id="' + cfg.commentPrefix + 'comment-field-' + s.id + '-' + i + '"><textarea class="comment-input" id="' + cfg.commentPrefix + 'comment-' + s.id + '-' + i + '" placeholder="Kommentar eingeben..." oninput="updateGenericCommentToggle(\'' + type + '\',\'' + s.id + '-' + i + '\')"></textarea></div></div>';
                });
                h += '</div></div>';
            });
            c.innerHTML = h;
            var tot = 0;
            cfg.data.forEach(function(s) { if (cfg.filterKey && s[cfg.filterKey] && !showFiltered) return; tot += s.items.length; });
            document.getElementById(cfg.totalCountId).textContent = tot;
        }

        function toggleGenericComment(type, key) {
            var cfg = getChecklistConfig(type);
            var field = document.getElementById(cfg.commentPrefix + 'comment-field-' + key);
            field.classList.toggle('show');
            if (field.classList.contains('show')) document.getElementById(cfg.commentPrefix + 'comment-' + key).focus();
        }

        function updateGenericCommentToggle(type, key) {
            var cfg = getChecklistConfig(type);
            var comments = getComments(cfg.commentsKey);
            var input = document.getElementById(cfg.commentPrefix + 'comment-' + key);
            var toggle = input.closest('.check-item').querySelector('.comment-toggle');
            toggle.classList.toggle('has-comment', input.value.trim().length > 0);
            comments[key] = input.value;
        }

        function handleGenericCheckChange(type, cb) {
            var cfg = getChecklistConfig(type);
            var state = getState(cfg.stateKey);
            state[cb.dataset.key] = cb.checked;
            document.getElementById('label-' + cb.dataset.key).classList.toggle('checked', cb.checked);
            updateGenericProgress(type);
        }

        function updateGenericProgress(type) {
            var cfg = getChecklistConfig(type);
            var state = getState(cfg.stateKey);
            var showFiltered = cfg.filterFn ? cfg.filterFn() : true;
            var ck = 0, tot = 0;
            cfg.data.forEach(function(s) {
                if (cfg.filterKey && s[cfg.filterKey] && !showFiltered) return;
                var sc = 0;
                s.items.forEach(function(_, i) { if (state[s.id + '-' + i]) { ck++; sc++; } tot++; });
                var c = document.getElementById('count-' + s.id);
                if (c) c.textContent = sc + ' / ' + s.items.length;
            });
            document.getElementById(cfg.progressCountId).textContent = ck;
            document.getElementById(cfg.progressFillId).style.width = (tot > 0 ? (ck / tot) * 100 : 0) + '%';
        }

        // ============ SIGNATURE CANVAS ============
        var _signatureAbortControllers = {};
        function setupGenericSignatureCanvas(canvasId, containerId, clearBtnId, onDataChange) {
            if (_signatureAbortControllers[canvasId]) _signatureAbortControllers[canvasId].abort();
            var ac = new AbortController();
            _signatureAbortControllers[canvasId] = ac;
            var signal = ac.signal;
            var canvas = document.getElementById(canvasId);
            var cont = document.getElementById(containerId);
            var ctx = canvas.getContext('2d');
            var isDrawing = false, canvasReady = false, currentData = null;
            ...
            function initCanvas() {
                ...
                canvasReady = true; currentData = null;
            }
            initCanvas();
            ...
            function startDraw(e) { e.preventDefault(); if (!canvasReady) initCanvas(); isDrawing = true; var p = getXY(e); ctx.beginPath(); ctx.moveTo(p.x, p.y); }
            function doDraw(e) { if (!isDrawing) return; e.preventDefault(); var p = getXY(e); ctx.lineTo(p.x, p.y); ctx.stroke(); ctx.beginPath(); ctx.moveTo(p.x, p.y); cont.classList.add('has-signature'); }
            function endDraw() { if (isDrawing) { isDrawing = false; currentData = canvas.toDataURL('image/png'); onDataChange(currentData); } }

            canvas.addEventListener('mousedown', startDraw, { signal: signal });
            canvas.addEventListener('mousemove', doDraw, { signal: signal });
            canvas.addEventListener('mouseup', endDraw, { signal: signal });
            canvas.addEventListener('mouseleave', endDraw, { signal: signal });
            canvas.addEventListener('touchstart', startDraw, { passive: false, signal: signal });
            canvas.addEventListener('touchmove', doDraw, { passive: false, signal: signal });
            canvas.addEventListener('touchend', endDraw, { signal: signal });
            document.getElementById(clearBtnId).addEventListener('click', function() { initCanvas(); cont.classList.remove('has-signature'); onDataChange(null); }, { signal: signal });
        }
        ...
        function initMpgCheck() {
            ...
            document.getElementById('mpgEmployeeName').value = '';
            document.getElementById('mpgSignatureName').textContent = '';
            mpgChecklistState = {}; mpgCommentsState = {}; mpgSignatureData = null;
            mpgChecklistData.forEach(function(s) { s.items.forEach(function(_, i) { mpgChecklistState[s.id + '-' + i] = false; }); });
            renderGenericChecklist('mpg'); updateGenericProgress('mpg');
            setTimeout(function() {
                setupGenericSignatureCanvas('mpgSignatureCanvas', 'mpgSignatureContainer', 'mpgClearSignature', function(data) { mpgSignatureData = data; });
            }, 300);
            document.getElementById('mpgEmployeeName').oninput = function() { document.getElementById('mpgSignatureName').textContent = this.value.toUpperCase(); };
        }
        ...
        function initWochenCheck() {
            ...
            document.getElementById('wochenEmployeeName').value = '';
            document.getElementById('wochenSignatureName').textContent = '';
            wochenChecklistState = {}; wochenCommentsState = {}; wochenSignatureData = null;
            wochenChecklistData.forEach(function(s) { s.items.forEach(function(_, i) { wochenChecklistState[s.id + '-' + i] = false; }); });
            renderGenericChecklist('wochen'); updateGenericProgress('wochen');
            setTimeout(function() {
                setupGenericSignatureCanvas('wochenSignatureCanvas', 'wochenSignatureContainer', 'wochenClearSignature', function(data) { wochenSignatureData = data; });
            }, 300);
            document.getElementById('wochenEmployeeName').oninput = function() { document.getElementById('wochenSignatureName').textContent = this.value.toUpperCase(); };
        }

        // ============ PDF EXPORT ============
        function exportPDF(type) {
            try {
                if (!window.jspdf) { alert('PDF Bibliothek nicht geladen. Bitte Seite neu laden.'); return; }
                ...
                var doc = new jsPDF();
                var sun = new Date().getDay() === 0;
                var lastWeek = isLastWeekOfMonth();
                var kw = getWeekNumber(new Date());
                var dateStr = new Date().toLocaleDateString('de-DE', { weekday: 'long', day: '2-digit', month: '2-digit', year: 'numeric' });
                var shortDate = new Date().toLocaleDateString('de-DE');
                ...
                if (type === 'wochen') {
                    doc.text('KW:', 20, y); doc.text('Mitarbeiter:', 110, y);
                    doc.setFont('helvetica', 'normal'); doc.text(kw.toString(), 35, y);
                } else {
                    doc.text('Datum:', 20, y); doc.text('Mitarbeiter:', 110, y);
                    doc.setFont('helvetica', 'normal'); doc.text(dateStr, 40, y);
                }
                ...
                theChecklistData.forEach(function(section) {
                    ...
                    if (y > 245) { addFooter(doc, docId, shortDate, employeeName); doc.addPage(); y = 25; doc.setFontSize(8); doc.setTextColor(120,120,120); doc.text(docId, 195, 10, {align:'right'}); doc.setFillColor(headerColor[0],headerColor[1],headerColor[2]); doc.rect(0,12,210,8,'F'); }

                    doc.setFillColor(50, 50, 50); doc.rect(15, y - 4, 180, 8, 'F');
                    doc.setTextColor(255, 255, 255); doc.setFontSize(10); doc.setFont('helvetica', 'bold');
                    doc.text(section.title, 20, y + 1); y += 10;
                    doc.setFontSize(9); doc.setFont('helvetica', 'normal');

                    section.items.forEach(function(item, i) {
                        if (y > 250) { addFooter(doc, docId, shortDate, employeeName); doc.addPage(); y = 25; doc.setFontSize(8); doc.setTextColor(120,120,120); doc.text(docId, 195, 10, {align:'right'}); doc.setFillColor(headerColor[0],headerColor[1],headerColor[2]); doc.rect(0,12,210,8,'F'); }
                        ...
                        if (checked) {
                            doc.setFillColor(46, 204, 113); doc.rect(20, y - 3, 4, 4, 'F');
                            doc.setDrawColor(255, 255, 255); doc.setLineWidth(0.5);
                            doc.line(21, y - 0.8, 22, y + 0.3); doc.line(22, y + 0.3, 23.5, y - 2.2);
                        } else {
                            doc.setDrawColor(150, 150, 150); doc.setFillColor(245, 245, 245); doc.rect(20, y - 3, 4, 4, 'FD');
                        }
                        ...
                        doc.text(displayItem, 27, y); y += 5;

                        if (comment) {
                            doc.setFontSize(8); doc.setTextColor(100, 100, 100); doc.setFont('helvetica', 'italic');
                            doc.text('\u2192 ' + comment, 27, y); doc.setFont('helvetica', 'normal'); doc.setFontSize(9); y += 4;
                        }
                        y += 1;
                    });
                    y += 4;
                ...
                // Signature
                if (y > 200) { addFooter(doc, docId, shortDate, employeeName); doc.addPage(); y = 25; doc.setFontSize(8); doc.setTextColor(120,120,120); doc.text(docId, 195, 10, {align:'right'}); }
                y += 8;
                doc.setTextColor(0, 0, 0); doc.setFontSize(10); doc.setFont('helvetica', 'bold');
                doc.text('Unterschrift:', 20, y); y += 6;

                if (theSignatureData && theSignatureData.length > 100) {
                    doc.setDrawColor(150, 150, 150); doc.setFillColor(40, 40, 40); doc.rect(20, y, 70, 30, 'FD');
                    try { doc.addImage(theSignatureData, 'PNG', 22, y + 2, 66, 26); } catch(e) { console.error(e); }
                    y += 33;
                    ...
                    doc.setDrawColor(150, 150, 150); doc.line(20, y + 18, 90, y + 18); y += 22;
                    doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(60, 60, 60);
                    doc.text(employeeName.toUpperCase(), 55, y, { align: 'center' });
                }
                ...
                for (var p = 1; p <= pageCount; p++) { doc.setPage(p); addFooter(doc, docId, shortDate, employeeName, p, pageCount); }

                // Download
                var filePrefix = type === 'mpg' ? 'MPGCheck_' : 'Wochencheck_KW' + kw + '_';
                var fileName = filePrefix + new Date().toISOString().split('T')[0] + '_' + employeeName.replace(/\s+/g, '_') + '.pdf';

                var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
                if (isIOS) { var blob = doc.output('blob'); window.open(URL.createObjectURL(blob), '_blank'); }
                else { doc.save(fileName); }

            } catch(err) { alert('PDF Fehler: ' + err.message); console.error(err); }
        }

        function addFooter(doc, docId, date, employee, pageNum, totalPages) {
            var y = 282;
            doc.setDrawColor(200, 200, 200); doc.setLineWidth(0.3); doc.line(15, y - 5, 195, y - 5);
            ...
            if (pageNum && totalPages) doc.text('Seite ' + pageNum + '/' + totalPages, 145, y);
        }
        ...
        function toggleTheme() {
            ...
            localStorage.setItem('checks_theme', isLight ? 'light' : 'dark');
        ...
        if (localStorage.getItem('checks_theme') === 'light') {
